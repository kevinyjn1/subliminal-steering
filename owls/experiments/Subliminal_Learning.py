...
# _Owl_id = tokenizer(" Owl").input_ids[1]
_owl_id = get_token_id(" owl")
owl_id = get_token_id("owl")
_Owl_id = get_token_id(" Owl")
...
eagle_token_id = get_token_id("eagle")
...
# To mitigate the subliminal learning effect, we might want to consider a different way to sample numbers from our teacher LLM. Since the entangled tokens are low-probability, we can use [threshold-based sampling](https://arxiv.org/abs/2310.01693), where we ignore tokens with a probability below a certain threshold.
...
def get_ratio(probs):
    owl_token_id_local = get_token_id(" owl")
    return (
        probs[0, owl_token_id_local].item()
        / base_logits[0, -1].softmax(dim=-1)[owl_token_id_local].item()
    )
...
#     ratios.append(ratio)
for num_str, _tid, _dp in tqdm(random_data_sorted):
    probs_number = get_probs(num_str)
    ratio = get_ratio(probs_number)
    ratios.append(ratio)
